...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    # initialize a dictionary of explored steps
    explored = {}
    if not isinstance(curState[0], int): # if curState is a tuple of tuples
        curState = curState[0] # we only want (x,y) position from it
    explored[curState] = 0
    ...
    while 1:
        ...
        # if frontier is not empty, traverse the graph 
            ...
            for successor in allSuccessors:
            	...
            	# in case the state contains more than x,y coords
            	# make sure we are only using the x,y coords as a tuple
                aSuccessor = successor[0]
                if isinstance(aSuccessor, int): # this means state was just (x,y)
                    aSuccessor = successor[0]
                ...
                if explored.get(aSuccessor) is None or fCost < explored[aSuccessor]:
                    ...
                    frontier.update((successor[0], route, hCost), fCost)
